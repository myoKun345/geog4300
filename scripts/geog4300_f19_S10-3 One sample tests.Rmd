...
ggplot(sample20,aes(x=value))+geom_histogram(binwidth=0.5)
```
...
```{r}
sample50<-as.tibble(rnorm(50, mean=24, sd=2))
...
```{r}
t.test(sample50$value,mu=25,alternative="less") #Is the sample mean *less* than 25?
t.test(sample50$value,mu=25,alternative="greater") #Is the sample mean *greater* than 25?
```
...
```{r}
daymet_ahn <- daymet_data %>% filter(gisjn_cty=="G13059" & year==2013)
t.test(daymet_ahn$tmax, mu=23.1)
```
