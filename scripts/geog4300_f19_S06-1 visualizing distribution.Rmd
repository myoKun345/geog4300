...
census_ins<-census_data %>%
  group_by(St_name, Region) %>%
  summarise(nat_ins_mean=mean(nat_ins_pct),
            nat_ins_med=median(nat_ins_pct))

ggplot(census_ins, aes(x=nat_ins_mean, y=nat_ins_med, color=Region)) +
  geom_point()
```
...
```{r}
ggplot(census_data,aes(x=nat_ins_pct))+
  geom_histogram()+
  facet_wrap(~Region)
```
...
```{r}
ggplot(census_data,aes(x=nat_ins_pct, fill=Region))+
